...
export async function searchBestBuy(category?: string, priceRange?: string) {
  ...
  return filteredProducts.map((product, index) => {
    ...
    return {
      ...
      image: `https://via.placeholder.com/300x200/0066CC/FFFFFF?text=${encodeURIComponent(product.name.split(' ').slice(0, 2).join(' '))}`,
      partner: "Best Buy",
      affiliate_url: affiliateLink,
...
export async function searchTarget(category?: string, department?: string) {
  ...
  return filteredProducts.map((product, index) => {
    ...
    return {
      ...
      rating: product.rating,
      reviews: `${product.reviews.toLocaleString()} reviews`,
      image: `https://via.placeholder.com/300x200/CC0000/FFFFFF?text=${encodeURIComponent(product.name.split(' ').slice(0, 2).join(' '))}`,
      partner: "Target",
      affiliate_url: affiliateLink,
      category: product.category,
